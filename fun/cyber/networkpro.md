...
[socket](./WebApplication.md#socket)

### what is port

端口属于**传输层**概念

- 端口号是由**传输层**的TCP或UDP协议给应用层的程序分配的
- 在表示端口时，应标明是TCP端口还是UDP端口
...
- 只有应用层的网络程序才会有端口
- ping命令使用网络层的ICMP协议，就不需要端口。

### 端口分类

在TCP/IP协议中用16位二进制数表示端口号，十进制取值范围为0~65535，其中0端口未
用，可用的端口号范围是1~65535。
...
客户端采用随机端口
- 当某个应用层的程序需要通过网络传输数据时，系统就为它分配一个随机端口。
- 当数据传输完毕，与服务器之间的连接断开，端口就被自动收回，可以再分配给其它程序使用。

服务端采用固定端口，固定端口包含两层含义：
- 第一，这个端口是固定分配给某个程序使用的。
- 第二，固定端口不会自动关闭，即使没有任何数据在传输，这个端口也会一直开放着。
