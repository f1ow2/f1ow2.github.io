---
layout: home


hero:
    tagline: Website
    actions:
      - theme: brand
        text: web
        link: website.md#web
      - theme: alt
        text: hash
        link: website.md#hash
      - theme: brand
        text: tools
        link: website.md#tools    
      - theme: alt
        text: Privilege Escalation
        link: website.md#privilege-escalation    
features:
  - title: TryHackMe
    icon: 
      src: /icon/tryhackme.png
    details: TryHackMe is a free online platform for learning cyber security, using hands-on exercises and labs, all through your browser!
    link: https://tryhackme.com/
  - title: FMHY
    icon: 
      src: /icon/FMHY.png
    details: The largest collection of free stuff on the internet!.
    link: https://fmhy.net/beginners-guide
  - title: HackTricks
    icon: 
      src: https://book.hacktricks.wiki/zh/images/hacktricks.gif
    details:  trick/technique/whatever in CTFs, real life apps, and reading researches and news.
    link: https://book.hacktricks.xyz/
  - title: CyberChef
    icon: 
      src: /icon/cyberchef.svg
    details: CyberChef is a simple, intuitive web app for carrying out all manner of "cyber" operations within a web browser.
    link: https://gchq.github.io/CyberChef/
---

<p align="center">
  <img src="/frieren.jpg" alt="frieren" />
</p>

---

# Website

## web

* [Exploit Notes](https://exploitnotes.org)
  
* [Wayback Machine](https://web.archive.org/)

* [withsecure](https://labs.withsecure.com/home)

* [dnsdumpster](https://dnsdumpster.com/)

* [shodan.io](https://www.shodan.io/)

* [Security Headers](https://securityheaders.com/)

* [javascript obfuscator](https://codebeautify.org/javascript-obfuscator)

* [javascript deobfuscate](https://obf-io.deobfuscate.io/)

* [pentestmonkey](https://pentestmonkey.net/)

* [Exploit-DB](https://www.exploit-db.com/)

* [Open Web Application Security Project](https://owasp.org/)

* [Certificate Search](https://crt.sh/)

* [requestbin](https://pipedream.com/requestbin)
  
* [maclookup](https://maclookup.app/)
---

## hash

* [SRI Hash Generator](https://www.srihash.org/)

* [crackstation](https://crackstation.net/)

* [hash_identifier](https://hashes.com/en/tools/hash_identifier)

* [sha256algorithm](https://sha256algorithm.com/)

---

## tools

* [AppDevTools](https://appdevtools.com/)

* [CyberChef](https://gchq.github.io/CyberChef/)
  
* [Reverse Shell](https://www.revshells.com/)

* [base64decode](https://www.base64decode.org/)

* [Wappalyzer](https://www.wappalyzer.com/)

* [JWT.io](https://jwt.io/)

* [Logto JWT](https://logto.io/jwt-decoder)

* [SSRFmap](https://github.com/swisskyrepo/SSRFmap)

* [HackTools](https://github.com/LasCC/HackTools)

---

## Privilege Escalation 

* [GTFOBins](https://gtfobins.github.io/)

* [GrimBins](https://grimbins.github.io/)
